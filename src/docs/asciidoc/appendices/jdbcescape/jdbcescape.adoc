...
Below you will find the list of JDBC functions and if they are supported by Jaybird.

=== Numeric Functions

[cols="3m,^1,5",options="header",]
|===
|JDBC
|built-in
|Description
...
|Xfootnote:[The `CHARACTERS` parameter only determines that characters are counted, the ignored blanks (space (0x20) or NUL (0x00)) are not determined by the parameter but by the underlying type]
