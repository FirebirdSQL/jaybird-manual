...
The properties marked as _boolean property_ can be included in the JDBC URL with values `true`, `false`, but also without a value, or with an empty value (which will both signify `true`).
...
The default of _boolean_ properties varies, but unless explicitly mentioned, the default is `false`.
...
a|`legacyAuthCharset` +
`legacyAuthCharSet`
|Jaybird specific property (since:[Jaybird 5.0.13/6.0.6]).
The Java character set to use when hashing the password for Firebird 2.5 and older, or for the `Legacy_Auth` plugin. +
Default: `UTF-8`.
...
a|Jaybird specific property.
Boolean property.
Blobs are created as stream (`true`) or segmented (`false`) blobs. +
Default: `true`
...
a|`scrollableCursor`
a|Jaybird specific property (since:[Firebird 5.0]).
...
a|`dataTypeBind` +
`set_bind`
|Defines data type bind from one type to a different type.
...
This property also configures the time zone used for legacy datetime conversion on all Firebird versions.
See <<ref-datetime-sessiontimezone>> for more information. +
Default: the JVM default time zone

a|`decfloatRound` +
`decfloat_round`
|Defines server-side rounding behaviour for `DECFLOAT` data types
...
a|`decfloatTraps` +
`decfloat_traps`
|Defines server-side error behaviour for `DECFLOAT` data types
...
a|`escapeProcessing`
a|JDBC standard property (since:[Jaybird 7]).
Boolean property.
Sets the default escape processing behaviour of `Statement` objects of a connection.
Default is `true`.
...
In addition, Jaybird allows using arbitrary Database Parameters Block entries as connection properties (provided they are defined in Jaybird's `DpbItems` and `SpbItems`).
...
If a DPB item requires another type, it will need to be explicitly defined in Jaybird to work correctly.
