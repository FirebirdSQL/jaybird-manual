...
Should be used on the systems where default socket buffer provided by JVM is not correct, or for tuning.
...
In the JNA implementation (native protocol) the connect timeout works as the DPB item `isc_dpb_connect_timeout` which only works after connecting to the server for the `op_accept` phase of the protocol.
...
Default: not set (Jaybird 3 and earlier), or the JVM default time zone.
