...
The properties marked as _boolean property_ can be included in the JDBC URL with values `true` or `false`, but also without a value, or with an empty value.
...
Allowed values: `DEFAULT`, `ENABLED`, `REQUIRED`, `DISABLED`.
