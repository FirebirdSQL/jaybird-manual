...
a|`timeZoneBind`
|Instructs Firebird to convert columns and parameters of type `TIME WITH TIME ZONE` and `TIMESTAMP WITH TIME ZONE` to a different type. ([.since]_Jaybird 3.0.6_ [.since]_Firebird 4_)
Accepted values are: `native`, `legacy`.
See <<ref-datetime-timezonebind>> for more information.
