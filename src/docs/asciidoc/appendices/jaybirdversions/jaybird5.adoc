...
For native or embedded connections, JNA 5.12.1 needs to be on the class path.
...
Where `<java>` is either `java8` or `java11`.
...
Artifactid: `jaybird`, +
Version: `{jaybird5-template-version}` (where `<java>` is either `java8` or `java11`)
