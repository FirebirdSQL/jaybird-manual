...
It also includes plugins `Legacy_Auth` that supports the pre-Firebird-3 authentication mechanism, and `Srp256`.
Firebird 4.0 introduced the plugins `Srp224`, `Srp384` and `Srp512`.

The `Srp` plugin uses SHA-1, the other Srp-variants use SHA-224, SHA-256, SHA-384 and SHA-512 respectively.footnote:[
...
For example, SHA-224 was not supported in Oracle Java 7 by default and required an additional JCE library.
====
...
When connecting to Firebird 3.0 or higher, the pure Java protocol in Jaybird will not try the `Legacy_Auth` plugin by default as it is an unsafe authentication mechanism.
