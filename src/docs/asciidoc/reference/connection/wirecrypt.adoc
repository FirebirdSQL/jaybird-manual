...
since:[Jaybird 5] On Java 11 or higher, Jaybird 5 and higher also support the `ChaCha` wire encryption. +
since:[Jaybird 6] Since Jaybird 6, the `ChaCha64` wire encryption is supported *if* the `chacha64-plugin` artifact and its Bouncy Castle dependency are on the classpath.
