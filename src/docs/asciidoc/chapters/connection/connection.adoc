...
If _serverName_ is not specified, it defaults to `localhost` (since:[Jaybird 5]). +
...
since:[Jaybird 5] Since Jaybird 5, all URLs supported by fbclient can be used.
...
====
since:[Jaybird 5] The `LOCAL` protocol was removed in Jaybird 5, and is now simply an alias for `NATIVE`.
...
When using Jaybird 3 and later, you can use a library to provide the Firebird client library for the `NATIVE` protocol.
...
====
since:[Jaybird 5] The OOREMOTE protocol implementation is deprecated and has been removed in Jaybird 6.
...
When the application is done with a connection, it should call `close()` on the connection.
...
The connection pool will either keep it for re-use, or close the physical connection.
