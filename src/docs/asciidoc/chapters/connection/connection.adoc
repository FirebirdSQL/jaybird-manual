...
The first part -- `jdbc:firebird:` or `jdbc:firebirdsql:` -- is required by JDBC.
It specifies the so-called protocol and subprotocol for the JDBC connection.
...
----
<1> This loads the driver (optional, see also next paragraphs)
<2> Creates a connection

The first line of this code used to be important -- it tells Java to load the Jaybird JDBC driver.
As required by the JDBC specification, at this point the driver registers itself with `java.sql.DriverManager`.

Since Java 6 (JDBC 4), explicitly loading the driver using `Class.forName("org.firebirdsql.jdbc.FBDriver")` is no longer necessary, except when the driver is not on the system (initial) classpath.
...
There, the driver is not on the system classpath, so it will need to be loaded explicitly.

[TIP]
====
Explicitly loading the driver with `Class.forName("org.firebirdsql.jdbc.FBDriver")` may also aid in debugging "`__No suitable driver found for jdbc:firebird...__`" errors:

. is the driver not found on the classpath, or
. it was found, but an error occurred during loading or initialization,
. or -- if the error goes away -- it was not loaded by automatic driver loading.
====
...
****
There are several ways to register a JDBC driver:

. `DriverManager` loads the drivers from the system classpath.
This happens automatically.
. The application explicitly loads the driver's class.
+
[source,java]
----
...
. The JDBC driver class name is listed in the `jdbc.drivers` system property.
...
The second statement of the example tells `java.sql.DriverManager` to open a database connection to the Firebird server running on localhost, and the path to the database is `c:/database/employee.fdb`.

The connection specification consists of -- optionally -- the host name of the database server (defaults to localhost), and optionally you can specify a port (defaults to 3050).
...
Jaybird also supports specifying connection properties in the JDBC URL.
...
The support for URL encoding _only_ applies to the JDBC URL part after the first `?`.
...
As mentioned in the section <<Jaybird Architecture>>, Jaybird supports multiple implementations of the GDS-ng API.
The default Jaybird distribution contains two main categories of the implementations: the Type 4 (pure Java) implementation of the Firebird wire protocol, and a Type 2 implementation using a https://github.com/java-native-access/jna[JNA (Java Native Access)^] proxy that can use a Firebird `fbclient` library.
...
[[driver-pure-java]]
==== PURE_JAVA (JDBC Type 4)

The `PURE_JAVA` type (JDBC Type 4) provides a pure Java implementation of the Firebird wire protocol.
This type is recommended for connecting to a remote database server using TCP/IP sockets.
No installation is required except adding the JDBC driver to the classpath.
...
When using `javax.sql.DataSource` implementation, you can specify either `"PURE_JAVA"` or `"TYPE4"` as the driver type.
This type is already used by default.
...
For example, say you have a server called `java`, then `jdbc:firebird:java:employee` will attempt to open the `employee` db alias on `localhost`, not on `java`, as `java:` is interpreted as the optional sub-subprotocol, and not as the hostname.
...
since:[Jaybird 6] Native connections require the `jaybird-native` artifact on the classpath.

The `NATIVE` type (JDBC Type 2) uses a JNA proxy to access the Firebird client library and requires installation of the Firebird client.
The `NATIVE` driver type is used to access a remote database server through TCP/IP, or -- Windows-only -- XNET or WNET.
Performance of the `NATIVE` driver using TCP/IP is approximately 10% lower compared to the `PURE_JAVA` driver, but with XNET has up to 30% higher performance compared to the `PURE_JAVA` driver when connecting to a server on the same host.
This is mostly due to the fact that TCP/IP stack is not involved in XNET.

In addition to Jaybird and -- since Jaybird 6 -- `jaybird-native`, this requires a native Firebird client library on the `PATH` or `LD_LIBRARY_PATH`, and JNA {jna-version} needs to be on the classpath.

To create a connection using the `NATIVE` JDBC driver to connect to a remote server you have to use the following JDBC URL syntax:
...
All URLs supported by fbclient can be used.
...
====
The `LOCAL` protocol was removed in Jaybird 5, and is now simply an alias for `NATIVE`.
...
You can use a library to provide the Firebird client library for the `NATIVE` protocol.
For Windows, Linux, and macOS, you can add the https://github.com/mrotteveel/jaybird-fbclient[`org.firebirdsql.jdbc:fbclient`^] dependency to your classpath.
...
Since version 5.0.1.1 of this library, you can also specify the desired OS, or OS and architecture using the `classifier`:
...
TIP: We recommend using the solution documented in <<driver-native-maven>>.

On Windows, you need to make sure that `fbclient.dll` is located on the `PATH` environment variable.
Alternatively, you can specify the directory containing this DLL in the `jna.library.path` system property or the <<connectionproperties-other,connection property>> ``nativeLibraryPath``footnote:first-native[This connection property will only work for the *first* native connection made.].
...
If your Java installation is 64-bit, you need a 64-bit `fbclient.dll`, for 32-bit Java, a 32-bit `fbclient.dll`.
...
On Linux, you need to make sure that `libfbclient.so` is available through the `LD_LIBRARY_PATH` environment variable.
...
Alternatively, you can specify the directory containing this DLL in the `jna.library.path` system property or the <<connectionproperties-other,connection property>> ``nativeLibraryPath``footnote:first-native[].
...
In practice, the URL accepts the same `<fbclient-url>` values as described for <<driver-native,`NATIVE`>>.
...
See also the <<driver-native>> section.
...
The `OOREMOTE` type is a JDBC Type 4 driver specifically for use with OpenOffice.org and LibreOffice.
...
This is not about the Firebird embedded use that has been introduced in recent LibreOffice versions.
====
...
====
The OOREMOTE protocol implementation is deprecated and has been removed in Jaybird 6.
...
Each time a connection is opened via `DriverManager` or `FBSimpleDataSource`, a new physical connection to the server is opened.
This physical connection is closed when the connection is closed.
To avoid the overhead of creating connections, you can use a connection pool to maintain a cache (pool) of open physical connections that can be reused.

Jaybird does not provide its own connection pool implementation.
...
Connection pool implementations, whether provided by a Java EE/Jakarta EE application server or a third-party library, are exposed as an implementation of the `javax.sql.DataSource` interface -- just like "`basic`" data sources.

The most important method exposed by this interface is the `getConnection()` method, which returns a connection based on the configuration of the data source.
...
This allows for swapping between a non-pooling and pooling data source in an application without any changes to the code using the data source.
====

When the application ("`user`") is done with a connection, it should call `close()` on the connection.
When from a non-pooling data source, the physical connection will be closed.
For a logical connection from a connection pool, `close()` will invalidate the logical connection (which will make it behave like a closed connection), and return the underlying physical connection to the connection pool.
The connection pool will either keep it in the pool for re-use, or close the physical connection (e.g. if the pool has too many idle connections).
...
The JDBC 2.0 specification introduced the `javax.sql.XADataSource` interface for access to connections that can participate in distributed transactions with a JTA-compatible transaction coordinator.
This gives applications the possibility to use two-phase commit to coordinate commits to multiple resource managers (e.g. multiple databases).

Just like `javax.sql.ConnectionPoolDataSource`, applications normally don't access an `XADataSource` implementation directly, instead it is used as a factory of connections for an XA-enabled data source.
To the application this is usually exposed as a `javax.sql.DataSource`.
