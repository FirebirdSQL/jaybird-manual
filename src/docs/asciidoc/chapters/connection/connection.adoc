...
deployment. There, the driver is not on the system classpath, so it will need to be loaded explicitly.
...
configuration, we suggest you not to use relative paths, and instead use an alias.
...
connection should behave the same from the perspective of the user.
====
