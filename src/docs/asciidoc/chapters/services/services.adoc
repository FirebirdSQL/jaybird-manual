...
3+a|NOTE: since:[Jaybird 5] `serverName` and `portNumber` are replacements for `host` and `port`, which have been deprecated for removal in Jaybird 6.
...
|backupGuid
|String
a|since:[Jaybird 4.0.4] Sets the GUID of a previous backup (requires Firebird 4.0 or higher).
...
|inPlaceRestore
|boolean
a|since:[Jaybird 4.0.4] Enables in-place restore (requires Firebird 4.0 or higher).
This allows incremental restore, for example for a read-only replica, or a hot standby.

|preserveSequence
|boolean
a|since:[Jaybird 5] Enables preserve sequence for restore or fixup (requires Firebird 4.0 or higher).
This preserves the GUID and replication sequence of the original database.

|cleanHistory
|boolean
a|since:[Jaybird 4.0.7] Enable the _clean history_ option (requires Firebird 4.0.3 or higher).
...
|keepDays
|int
a|since:[Jaybird 4.0.7] Number of days to keep backup history when `cleanHistory` is enabled.

|keepRows
|int
a|since:[Jaybird 4.0.7] Number of rows (including the new backup!) to keep backup history when `cleanHistory` is enabled.
...
|fixupDatabase()
a|since:[Jaybird 5] Perform the nbackup fixup operation.
