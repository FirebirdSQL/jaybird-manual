...
These are used to perform various administrative tasks in Firebird, e.g. <<services-backup-and-restore>>, <<services-maintenance>>, <<services-db-stats>>.
The set of API calls to perform such tasks are known under the name "`Services API`" or "`Services Manager`".
Additionally, client applications can use the Services API to get limited information about the server environment and configuration.

The execution of Services API calls can be viewed as a task triggered by the client application to be executed on the server.
The parameters passed in the calls are similar to the ones that are passed to equivalent command-line tools.
The processed parameter values are passed into the entry routines of the _gbak_, _gfix_, _gsec_ or _gstat_ utility.
The output of the utility is transmitted over the network to the client application -- instead of the standard out when run from the commandline.
...
|String
|Name or IP address of the host to which we make the Service API request. _Required_.
...
3+a|NOTE: `serverName` and `portNumber` are replacements for `host` and `port`, which were removed in Jaybird 6.

|database
|String
|Path to the database. 
The meaning of the property depends on the service being invoked and will be described in each of sections below.
...
|expectedDb
|String
a|With Firebird 3.0 and higher, this is used to find the non-default security database to use when authenticating.
...
|processId
|int
a|Process id to report to the server.

Native connections will always use the actual process id and ignore this property. +
since:[Jaybird 6] Since Jaybird 6, pure Java connections will default to the actual process id.

See also <<ref-processinfo>>.

|processName
|String
a|Process name to report to the server.

See also <<ref-processinfo>>.

|socketBufferSize
|int
a|Socket buffer size in bytes

|soTimeout
|int
a|Socket blocking read timeout in milliseconds (`0` uses OS default timeout)

|connectTimeout
|int
a|Socket connect timeout in milliseconds (`0` uses OS default timeout)
...
|wireCryptAsEnum
|WireCrypt
a|Alternative to `WireCrypt` to use `WireCrypt` enum.
...
The last parameter -- `logger` -- requires some explanation.
...
Jaybird supports <<services-backup-and-restore-gbak,server-side _gbak_>>, <<services-backup-and-restore-gbak-streaming,streaming _gbak_>>, and <<services-backup-and-restore-nbackup,_nbackup_>> backup and restore.
...
For details on _gbak_ itself, consult the https://firebirdsql.org/file/documentation/html/en/firebirddocs/gbak/firebird-gbak.html[Firebird Backup & Restore Utility^] documentation.

In addition to the setters and getters described in <<servicemanager>>, the following properties and methods are used to specify the backup and restore paths and other configuration:

[cols="2m,1m,4a",options="header",]
|===
|Name |Type |Description

|database
|String
|For backup, the path or alias of the database to back up. +
For restore, the path to the database into which the backup file will be restored.
To restore a multi-file database, use the `addRestorePath(String, int)` method instead.

|backupPath
|String
|For a backup, the path of the newly created backup file.
For multi-file backup files, use the `addBackupPath(String, int)` method instead. +
For a restore, the path to the single backup file.
...
|int
|Page buffers size to configure in the restored database (restore only).
If not set, the size recorded in the backup is used.

|restorePageSize
|int
|Database page size (restore only).
...
To avoid such situation it is recommended to restore a database into some dummy file first and then use filesystem commands to replace the existing database with the newly created one.
...
|clearBackupPaths()
|Clear all specified backup paths.

|clearRestorePaths()
|Clear all specified restore paths.
...
The `FBStreamingBackupManager` can be used to perform a backup and restore streaming the backup data to/from the client, see <<services-backup-and-restore-gbak-streaming>>.

After configuration, use methods `backupDatabase()`, `backupMetadata()` and `restoreDatabase()` to perform the backup or restore tasks.
...
...
// and restore it
var restoreManager = new FBBackupManager();
...
restoreManager.setRestoreReplace(true); // attention <1>
...
----
<1> Be careful with `restoreReplace`.
It will unconditionally drop (delete) the database before restore.
This can result in loss of your database if the restore fails, or if you pointed to the wrong database file.

The methods `backupDatabase(int)` and `restoreDatabase(int)` can be used to specify additional backup and restore options that cannot be specified via the properties of this class.
The parameter value is a bitwise OR of the following constants:

[cols="3m,4a",options="header",]
|===
...
By default, external tables are not backed up, only paths to the external data files are stored in the backup file.
...
When such situation occurs, the backup has to wait for the decision about the outcome of the in-limbo transaction.
After a wait timeout, an exception is thrown and the backup is aborted.
...
When this option is specified, the backup utility creates a backup of only the metadata information (e.g. table and view structures, stored procedures, etc.), but no data are backed up.
...
This process is time-consuming and can be switched off when creating a backup for immediate restore, reading only the latest record versions.

After backup, an operator can restore the database from the backup.
...
If a shadow database is configured, an absolute path to the shadow is stored in the backup file.
If such a backup is restored on a different system where the path does not exist (e.g. moving a database from Windows to Linux or otherwise), the restore will fail.
Using this option allows to overcome such situations.

|RESTORE_NO_VALIDITY 
|Do not restore validity constraints.

This option is usually needed when the validity constraints (e.g. `NOT NULL` constraints) were added after the data were already in the database, but the database contains records that do not satisfy such constraintsfootnote:[Firebird 2.5 and older allow you to define validity constraints despite the table(s) containing data that do not satisfy them.
 Only new records will be validated, and it is responsibility of the database administrator to ensure the validity of existing ones.].
...
|Do not reserve space on each data page for the future record versions, useful for read-only databases.
|===
...
For information about _nbackup_, consult the https://firebirdsql.org/file/documentation/html/en/firebirddocs/nbackup/firebird-nbackup.html[Firebird's nbackup tool^] documentation.

In addition to the setters and getters described in <<servicemanager>>, the following methods are used to specify the backup and restore paths and properties:
...
|database
|String
|For a backup, the path or alias of the database to back up. +
For a restore, the path to the database into which the backup file will be restored.

|backupFile
|String
|For a backup, the path of the newly created backup file on the server. +
For a restore, the path to a single backup file.
...
A level of `N` with `N > 0` will back up all data pages modified since the last level `N - 1` backup.
Defaults to `0` if `backupGuid` has not been set.

|backupGuid
|String
|Sets the GUID of a previous backup (requires Firebird 4.0 or higher).
This will back up all data pages modified since the backup identified by the GUID.
The GUID must be enclosed in braces (`++{++` and `++}++`).
...
|inPlaceRestore
|boolean
|Enables in-place restore (requires Firebird 4.0 or higher).
This allows incremental restore, for example for a read-only replica, or a hot standby.

|preserveSequence
|boolean
|Enables preserve sequence for restore or fixup (requires Firebird 4.0 or higher).
This preserves the GUID and replication sequence of the original database.

|cleanHistory
|boolean
|Enable the _clean history_ option (requires Firebird 4.0.3 or higher).
One of the properties `keepRows` _or_ `keepDays` must also be set.
When enabled, after performing a backup, old records from `RDB$BACKUP_HISTORY` will be removed.

|keepDays
|int
|Number of days to keep backup history when `cleanHistory` is enabled.

|keepRows
|int
|Number of rows (including the new backup!) to keep backup history when `cleanHistory` is enabled.
...
|fixupDatabase()
|Perform the _nbackup_ fixup operation.
...
After configuration, use the `backupDatabase()`, `restoreDatabase()` and `fixupDatabase` methods to perform the backup or restore tasks.
These methods will block until the operation is finished.
...
----
var backupManager = new FBNBackupManager();
...
The user management routines are defined in the `UserManager` interface and are implemented in the `FBUserManager` class.
Additionally, there is a `User` interface providing getters and setters for properties of a user account on the server and corresponding implementation in the `FBUser` class.footnote:[
...
----
var userManager = new FBUserManager();
...
=== Database maintenance [[_database_maintenance]]

Database maintenance is something that everybody would prefer to avoid, and, contrary to the backup/restore and user management procedures, there is little automation that can be done here.
Usually the maintenance tasks are performed on the server by the database administrator, but some routines are needed to perform automated database upgrade or perform periodic checks for database validity.
...
When a database is shut down, only the user that initiated the shutdown, either SYSDBA or the database owner, can connect to the database and perform other tasks, e.g. metadata modification or database validation and repair.

The database shutdown is performed with the `shutdownDatabase(int, int)` method.
The first parameter is the shutdown mode, the second is the maximum allowed time for operation.
...
[TIP]
====
Instead of shadows, consider using https://firebirdsql.org/file/documentation/release_notes/html/en/4_0/rlsnotes40.html#rnfb40-replication[replication], available since Firebird 4.0.
====
...
This is achieved by an algorithm called _careful writes_, which guarantees that the server writes data on disk in such a manner that despite events like a server crash, the database file remains in a consistent state.
...
This is the equivalent of https://firebirdsql.org/file/documentation/html/en/firebirddocs/gfix/firebird-gfix.html#gfix-db-validation[`gfix -validate`].
It also reports presence of checksum errors.
...
See also https://firebirdsql.org/file/documentation/html/en/firebirddocs/gfix/firebird-gfix.html#gfix-db-validate-noupdate[gfix: Read-only Validation].

|VALIDATE_FULL
|Do a full check on record and pages structures, releasing unassigned record fragments.

See also https://firebirdsql.org/file/documentation/html/en/firebirddocs/gfix/firebird-gfix.html#gfix-db-validate-full[gfix: Full Validation].
...
See also https://firebirdsql.org/file/documentation/html/en/firebirddocs/gfix/firebird-gfix.html#gfix-db-validate-ignore[gfix: Ignore Checksum Errors].

|===

To repair a corrupted database, use the `markCorruptRecords()` method which marks corrupted records as unavailable.
This method is equivalent to https://firebirdsql.org/file/documentation/html/en/firebirddocs/gfix/firebird-gfix.html#gfix-db-mend[`gfix -mend`].
After this operation, a database can be backed up and restored to a different place.
...
Limbo transactions -- or in-limbo transactions -- are transactions that were prepared for commit but were not (yet) committed or rolled back;
...
A limbo transactions affects the normal database operation, since the records modified in that transactions are not available, as Firebird does not know whether the new version will be committed or rolled back and blocks access to them.
...
Equivalent of https://firebirdsql.org/file/documentation/html/en/firebirddocs/gfix/firebird-gfix.html#gfix-transactions-list[gfix -list]
...
Equivalent of https://firebirdsql.org/file/documentation/html/en/firebirddocs/gfix/firebird-gfix.html#gfix-transactions-commit-rollback[gfix -commit _N_].

|rollbackTransaction(long)
|Rollback the transaction with the specified ID.

Equivalent of https://firebirdsql.org/file/documentation/html/en/firebirddocs/gfix/firebird-gfix.html#gfix-transactions-commit-rollback[gfix -rollback _N_].
...
Such transactions are marked as "rollback" transactions on the Transaction Inventory Page.
These transactions prevent advancing the so-called Oldest Interesting Transaction (OIT), the ID of the oldest transaction which created record versions that are relevant to any of the currently running transactions.
On each access to a record, Firebird has to check all record versions between the current transaction and the OIT, which leads to performance degradation on large databases.
To solve the issue, Firebird periodically starts a database sweeping process, that traverses all database records, removes changes made by rolled back transactions and moves forward the OIT.footnote:[
...
Equivalent of https://firebirdsql.org/file/documentation/html/en/firebirddocs/gfix/firebird-gfix.html#gfix-housekeeping-sweep[`gfix -housekeeping _N_`].

|sweepDatabase()
|Perform the sweep regardless of the current difference between Next Transaction and OIT.

Equivalent of https://firebirdsql.org/file/documentation/html/en/firebirddocs/gfix/firebird-gfix.html#gfix-housekeeping-manual-sweep[`gfix -sweep`].
...
|Change the access mode of the database.
Possible values are:

* `ACCESS_MODE_READ_ONLY` -- make database read-only;
* `ACCESS_MODE_READ_WRITE` -- allow writes into the database.

Only read-only databases can be placed on read-only media.
Read-write databases will need write access even if only accessed with read-only transactions.

Equivalent of https://firebirdsql.org/file/documentation/html/en/firebirddocs/gfix/firebird-gfix.html#gfix-dbmode[`gfix -mode`].

|setDatabaseDialect(int)
|Change the database SQL dialect.
Allowed values are 1 (deprecated) and 3.

Equivalent of https://firebirdsql.org/file/documentation/html/en/firebirddocs/gfix/firebird-gfix.html#gfix-dialect[`gfix -sql_dialect`].

|setDefaultCacheBuffer(int)
|Configure page buffers (page cache size) of the database.

This setting applies to this specific database, overriding the system-wide default.

Equivalent of https://firebirdsql.org/file/documentation/html/en/firebirddocs/gfix/firebird-gfix.html#gfix-buffers[`gfix -buffers`].

|setForcedWrites(boolean)
|Change the forced writes setting for the database.

When forced writes are switched off, the database engine does not enforce flushing pending changes to disk, so they might be kept in the OS cache.
If the same page is changed again later, the write may happen in memory, which in many cases increases performance.
However, in case of OS or hardware crashes, the database might get corrupted as some or all of the changes might not have been written to disk yet.

Equivalent of https://firebirdsql.org/file/documentation/html/en/firebirddocs/gfix/firebird-gfix.html#gfix-sync[`gfix -write`].

|setPageFill(int)
|Set the page fill factor.

By default, Firebird leaves some free space on each database page for future record versions.

It is possible to tell Firebird not to reserve this space.
This makes sense for read-only databases, since more data fits the page, which reduces database size and may increase performance.
For read-write database, this is likely to decrease performance.

Allowed values are:

* `PAGE_FILL_FULL` -- do not reserve additional space for future versions;
* `PAGE_FILL_RESERVE` -- reserve free space for future record versions.

Equivalent of https://firebirdsql.org/file/documentation/html/en/firebirddocs/gfix/firebird-gfix.html#gfix-pagespace[`gfix -use`].
...
[#services-db-stats]
=== Database statistics [[_database_statistics]]

The `StatisticsManager` interface and corresponding implementation in the `FBStatisticsManager` class can be used to obtain statistical information for the database, like page size, values of OIT and Next transactions, database dialect, database page allocation and its distribution.

The following methods provide the functionality equivalent to the https://firebirdsql.org/file/documentation/html/en/firebirddocs/gstat/firebird-gstat.html[_gstat_] command line tool.
The output of the commands are written to the output stream specified in the `logger` property.
...
Equivalent of https://firebirdsql.org/file/documentation/html/en/firebirddocs/gstat/firebird-gstat.html#gstat-cmdline-all[`gstat -all`].
...
* `DATA_TABLE_STATISTICS` (https://firebirdsql.org/file/documentation/html/en/firebirddocs/gstat/firebird-gstat.html#gstat-cmdline-data[`gstat -data`])
* `SYSTEM_TABLE_STATISTICS` (https://firebirdsql.org/file/documentation/html/en/firebirddocs/gstat/firebird-gstat.html#gstat-cmdline-system[`gstat -system`])
* `INDEX_STATISTICS` (https://firebirdsql.org/file/documentation/html/en/firebirddocs/gstat/firebird-gstat.html#gstat-cmdline-index[`gstat -index`])
* `RECORD_VERSION_STATISTICS` https://firebirdsql.org/file/documentation/html/en/firebirddocs/gstat/firebird-gstat.html#gstat-cmdline-record[`gstat -record`])

|getHeader{zwsp}Page()
|Get information from the header page (e.g. page size, OIT, OAT and Next transaction values, etc.)

Equivalent of https://firebirdsql.org/file/documentation/html/en/firebirddocs/gstat/firebird-gstat.html#gstat-cmdline-header[`gstat -header`].
...
since:[Jaybird 7] Method accepts varargs (`++String...++`).
In Jaybird 7 and later, specifying no tables will list all tables.

Equivalent of https://firebirdsql.org/file/documentation/html/en/firebirddocs/gstat/firebird-gstat.html#gstat-cmdline-table[`gstat -table`].

|getTable{zwsp}Statistics({zwsp}Collection<String>)
|since:[Jaybird 7] Same as previous, but as a collection of table names.

|getTable{zwsp}Statistics({zwsp}Collection<String>, Collection<String>)
a|since:[Jaybird 7] Get statistic information for the listed tables in the listed schemas.
