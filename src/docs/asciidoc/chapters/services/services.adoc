...
|soTimeout
|int
a|Socket blocking read timeout in milliseconds (`0` is OS default timeout)

|connectTimeout
|int
a|Socket connect timeout in milliseconds (`0` is OS default timeout)
...
Jaybird supports server-side _gbak_, streaming _gbak_, and _nbackup_ backup and restore.
...
After specifying the needed properties, the application developer can use methods `backupDatabase()`, `backupMetadata()` and `restoreDatabase()` to perform the backup and restore tasks.
...
For a restore operation, it specifies the path to a single backup file.
...
|backupGuid
|String
a|Sets the GUID of a previous backup (requires Firebird 4.0 or higher).
...
|inPlaceRestore
|boolean
a|Enables in-place restore (requires Firebird 4.0 or higher).
This allows incremental restore, for example for a read-only replica, or a hot standby.

|preserveSequence
|boolean
a|Enables preserve sequence for restore or fixup (requires Firebird 4.0 or higher).
This preserves the GUID and replication sequence of the original database.

|cleanHistory
|boolean
a|Enable the _clean history_ option (requires Firebird 4.0.3 or higher).
...
|keepDays
|int
a|Number of days to keep backup history when `cleanHistory` is enabled.

|keepRows
|int
a|Number of rows (including the new backup!) to keep backup history when `cleanHistory` is enabled.
...
|fixupDatabase()
a|Perform the _nbackup_ fixup operation.
...
After specifying the needed properties, the application developer can use the `backupDatabase()`, `restoreDatabase()` and `fixupDatabase` methods to perform the backup and restore tasks.
These methods will block until the operation is finished.
...
On each access to the records, Firebird has to check all record versions between the current transaction and the OIT, which leads to performance degradation on large databases.
...
This makes sense for read-only databases, since more data fits the page, which reduces database size and increases performance.
For read-write database, this is more likely to decrease performance.
