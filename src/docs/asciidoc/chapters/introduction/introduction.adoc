...
New or removed features are tagged with the version that introduced a feature (e.g. [.since]_Jaybird 3_) or removed feature (e.g. [.until]_Jaybird 3_).
This tagging is only done for features introduced (or removed) in Jaybird 2.2 or later, or in Firebird 3.0 or later, and for features available in Java 8 (JDBC 4.2) or later.
