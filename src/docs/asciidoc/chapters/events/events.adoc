...
3+a|NOTE: `serverName`, `portNumber` and `databaseName` are replacements for `host`, `port` and `database`, which were removed in Jaybird 6.
...
|expectedDb
|String
|With Firebird 3.0 and higher, this is used to find the non-default security database to use when authenticating.
...
|processId
|int
|Process id to report to the server.

|processName
|String
|Process name to report to the server.
...
|soTimeout
|int
|Socket blocking read timeout in milliseconds (`0` is OS default timeout)

|connectTimeout
|int
|Socket connect timeout in milliseconds (`0` is OS default timeout)
...
|wireCryptAsEnum
|WireCrypt
|Alternative to `WireCrypt` to use `WireCrypt` enum.
