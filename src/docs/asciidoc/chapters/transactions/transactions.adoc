...
Each newly created connection by default has auto-commit mode enabled by default.
...
until:[Jaybird 6] In Jaybird 5 and earlier, in auto-commit mode, a `TYPE_FORWARD_ONLY` result set was implicitly closed when `next()` returned false.
