...
This allows applications that work via the native Firebird API to reduce the number of network connections required, which in turn can save resources on the serverfootnote:[
Additionally, before InterBase was open-sourced, this allowed application developers to create multithreaded application without need to purchase additional user licenses.].

This model however is not used by all database engines, and the designers of the JDBC API have chosen a model where each database connection has only one active transaction.
Also, unlike the Firebird model, where transactions require an explicit start, the JDBC specification requires the driver to start a transaction automatically as soon as a transactional context is needed.

The following code shows a very simple example of using transactions in JDBC where a hypothetical intruder increases salary of each employee twice and uses explicit transaction control in JDBC.
They also try to hide their identity and if the operations succeeds, they commit the transaction, otherwise they roll the changes back.
...
To use transactions, the application first switches auto-commit mode off (see below for more information), then creates a `java.sql.Statement` object, and executes an `UPDATE` statement.
...
Alternatively, you can use an external table to store the audit log, or use the trace facility to log such operations.
====

When a connection uses explicit transaction control (auto-commit disabled), each transaction must be finished by calling the `commit()` or `rollback()` methods of the `Connection` object before the connection is closed.
If a transaction was not finished, but the `close` method is called, the active transaction is rolled back automatically.
If you do not commit or roll back the transaction, and do not close the connection explicitly, the rollback will only occur after the connection has been garbage collected, when the server detects the connection is gone.

[WARNING]
====
Given the non-deterministic delay of garbage collection and detection of terminated connections by the server, make sure to explicitly end the transaction and close the connection.
Do not rely on the garbage collector for this.
====

=== Auto-commit mode

Each newly created connection by default has auto-commit mode enabled.
...
until:[Jaybird 6] In Jaybird 5 and earlier, in auto-commit mode, a `TYPE_FORWARD_ONLY` result set is implicitly closed when `next()` returned false.
...
since:[Jaybird 6] In auto-commit mode, a result set will now remain open until explicitly closed using `ResultSet.close()`, when any statement is executed, when the auto-commit mode is disabled, or by the close of the `Statement` or `Connection`.
...
Note, when a connection is obtained via a `javax.sql.DataSource` object and container managed transactions are used (for example, the application is executing inside an EJB container, or the connection is participating in a JTA-managed distributed transaction), it is an error to call `setAutoCommit`.
...
Since Firebird does not allow a result set to remain open after the transaction ends, Jaybird 1.5.x and below cached the complete result set in memory when `SELECT` statements were executed and the transaction was committed.
This had an adverse effect on allocated memory when the result set is big, especially when it contains `BLOB` fields.
The JDBC 3.0 specification addressed this unclear situation (see above) and Jaybird 2.1 was improved to correctly handle this.
...
However, some Java applications that do not conform the current JDBC specifications might no longer work with Jaybird 2.1 and above unless additional steps are taken.

The piece of code below works perfectly with explicit transaction control.
However, it won't work correctly with auto-commit with a driver -- like Jaybird -- that complies with the JDBC 3.0 and higher specification, when the `selectStmt` and `updateStmt` object are created by the same connection object (step 1).
...
When the Java application tries to fetch the next record by calling the `rs.next()` method, it will receive an `SQLException` with a message "`__The result set is closed__`".

The correct solution to this situation is to fix the application by either using explicit transaction control, or by using two connection objects, one for the `SELECT` statement and one for the `UPDATE` statement.
...
To address this, Jaybird 2.1 introduced the connection parameter <<ref-defaultholdable,`defaultHoldable`>> which makes result sets holdable by default.
Holdable result sets are fully cached in memory, and will not be closed automatically when a transaction ends.footnote:[
...
The SQL standard defines four isolation levels, each next one weaker than the previous.
...
Transactions with this isolation level prohibit phantom reads, the situation when one transaction reads all rows satisfying a `WHERE` condition, another transaction inserts a row satisfying that condition, and the first transaction re-executes the statement.
...
The weakest isolation level, or better, a level without isolation.
Such transactions can see the not yet committed changes from concurrently running transactions.

Firebird, however, defines other isolation levels: `read_committed`, `concurrency` and `consistency`.
Only the `read_committed` isolation level can be mapped to the same level defined by the SQL standard.
Dirty reads are prevented, non-repeatable reads and phantom reads can occur.

The `concurrency` isolation level is stronger than repeatable read isolation defined in the SQL standard and satisfies the requirements of a serializable isolation level, however, unlike RDBMSes with locking concurrency control, it guarantees better performance.

And finally, Firebird provides a `consistency` isolation level which in combination with table reservation feature guarantees a deadlock-free execution of transactions.
A transaction will be prevented from starting if there is already another one with an overlapping sets of reserved tables.
This isolation level guarantees truly serial history of transaction execution.

To satisfy the JDBC specification Jaybird provides the following default mapping of the JDBC transaction isolation levels to Firebird isolation levels:
...
Attempts to call the `rollback(Savepoint)` method with a released savepoint will result in an `SQLException`.
...
Additionally, if the connection is set to read-only mode, this is reflected in the TPB.
...
This is controlled through the https://firebirdsql.org/file/documentation/html/en/refdocs/fbconf/firebird-configuration-reference.html#fbconf-read-consistency[`ReadConsistency`^] setting in `firebird.conf` or -- per database -- in `databases.conf`.
====
...
[NOTE]
====
With the `READ COMMITTED CONSISTENCY` mode introduced in Firebird 4.0, even read-only transactions will inhibit garbage collection.
====
...
The Firebird syntax also defines a number of https://firebirdsql.org/file/documentation/chunk/en/refdocs/fblangref50/fblangref50-transacs.html#fblangref50-transacs-statements[transaction management statements].
In this section we discuss the `COMMIT [WORK]`, `ROLLBACK [WORK]` and `SET TRANSACTION` statements, which have a hard transaction boundary (either starting or ending a transaction).

In general, you should not execute these statements with Jaybird.
...
In Jaybird 5 and older, the `COMMIT` and `ROLLBACK` statements can -- sometimes -- be executed, but they bring the connection in an inconsistent state, which will prevent further use of the connection.
Execution of the `SET TRANSACTION` statement will always fail, as the statement is executed with an active transaction.
