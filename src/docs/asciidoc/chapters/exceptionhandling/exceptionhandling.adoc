...
since:[Jaybird 6] Some applications log or report `SQLWarning` in a way that can be confusing or annoying.
...
** until:[Jaybird 6] `org.firebirdsql.jdbc.FBDriverConsistencyCheckException` -- this exception is thrown when driver detects an internal inconsistent state.
SQL state is `HY000`.
